...
$xy = array();
$currentXY = array('x' => 0, 'y' => 0);
$currentRXY = array('x' => 0, 'y' => 0);
$isRobot = false;
//Add the first position
$xy[] = "0,0";

foreach ($inputArray as $key => $direction) {
	if ($isRobot) {
		$currentRXY = xySwitch($direction, $currentRXY);
		$xy[] = $currentRXY['x'] . ',' . $currentRXY['y'];
		$isRobot = false;
	} else {
		$currentXY = xySwitch($direction, $currentXY);
		$xy[] = $currentXY['x'] . "," . $currentXY['y'];
		$isRobot = true;
	}
	
}

$houses = count(array_unique($xy));
echo "How many houses receive at least one present?\n";
echo "Nb of Houses is " . $houses . "\n";

function xySwitch($direction, $current) {
	switch ($direction) {
		case ">":
			$current['x'] += 1;
			break;
